...
bot.on('message', async(message) => {
	...
	if(isCommand == true){
		...
		if (checkMessage[0] == "~poll" && checkMessage[1] || checkMessage[0] == "~poll" && !checkMessage[1] == '' || checkMessage[0] == "~poll" && !checkMessage[1] == ' '){
			...
			var pollAryAsString = pollParts.toString();
			var theAnswers = [];
			...
			var pollAryAnswersN = theNumbers.toString().replace(/,/g, "");
			var pollAryAnswers = theAnswers.toString().replace(/,/g, "\n");
			var trueAnswers = `${pollAryAnswersN})${pollAryAnswers}`;
			const answerMap = new Map();
			
			answerMap
			.set('CH', { name: 'Confoederatio Helvetica', population: '8M' })
			.set('someothervalue', {name: 'SomeOtherName', population: '10M'});
			let b = 1;
for(const answ of answerMap.values()) {
    console.log(`${b}. ${answ.name} has a population of ${answ.population}`);
    b++;
}
			/*
			theAnswers.forEach(function(element){
					theNumbers.forEach(function(element2){
...
Q: ${pollQ}`;
			...
			for(i = 0; i < totalCount; i++){
				...
				var someNumbers = theAnswers.indexOf(theAnswers[0]);
				var counter = i + 1;
				/*
				const answers = new Map();
				for(i = 1; i <= totalCount; i++){
					answers.set('${theAnswers[i]}', {numbero: `${i}`,  response: `${theAnswers[i]}`});
				}
				let k = 1;
				for (const answer of answers.values()) {
					consonle.log(`${k}. ${answer.numbero} ${answer.response}`);
					k++;
				}*/	
...
${counter})${theAnswers[i]}`);*/
				message.channel.send(questionPreface + "\n\n" + newAnswers);
		}
	}
	/*
	const countries = new Map();
countries.set('CH', { name: 'Confoederatio Helvetica', population: '8M' });

let i = 1;
for(const country of countries.values()) {
    console.log(`${i}. ${country.name} has a population of ${country.population}`);
    i++;
}
...
for(const [ key, value ] of countryMap) {
	...
	*foreach ($counties as $k=>$county){
        list($county,$state) = explode(', ', $county);
        $population = $populations[$k];
        //echo "county = $county & population = $population" . "\n";
        //echo "UPDATE vtiger_researchcf SET cf_county_population = $population WHERE cf_county_name = $county;" . "\n";
        $adb->pquery("UPDATE vtiger_researchcf SET cf_county_population = $population WHERE cf_county_name = $county;");
	}
	*/
